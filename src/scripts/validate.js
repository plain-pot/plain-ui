...
const $validator = {
    ...
    rules: [
        ...
        },
        {
            ...
            execute(val, params) {
                ...
                let [min = '?', max = '?'] = params
                if (min !== '?' && (!val || val.length < (min - 0))) return {validate: false, msg: `长度不能小于${min}`}
                if (max !== '?' && !!val && val.length > (max - 0)) return {validate: false, msg: `长度不能大于${max}`}
        ...
        },
        {
            ...
            execute(val) {
                ...
                const flag = regexp.test(val)
                return {validate: flag, msg: flag ? null : '电话号码格式不正确',}
            },
        },
        {
            /*qq号码*/
            ...
            execute(val) {
                ...
                const flag = regexp.test(val)
                return {validate: flag, msg: flag ? null : 'qq号码格式不正确',}
            },
        },
        {
            /*身份证号*/
            ...
            execute(val) {
                ...
                return {validate: flag, msg: flag ? null : '身份证号格式不正确',}
            },
        },
        {
            /*自定义正则表达式*/
            ...
            execute(val, params) {
                ...
                const flag = regexp.test(val)
                return {validate: flag, msg: flag ? null : params[0],}
            },
        },
    ],
    /**
    ...
    analysisRuleName(ruleName) {
        let [name, params] = ruleName.split(validateSeparator)
