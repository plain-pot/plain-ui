...
export const TreeProps = {
    data: {type: Array as any as new() => any[]},               // 树形结构数据
    loading: {type: Boolean},                                   // 当前是否处于loading状态
    nodeIcon: {type: Function as any as new() => TreePropsType["nodeIcon"]},// 节点图标
    ...
    // 普通属性
    renderContent: {type: Function as any as new() => TreePropsType["renderContent"]},// 树节点内容渲染函数
    filterNodeMethod: {type: Function as any as new() => TreePropsType["filterNodeMethod"]},// 对树节点进行筛选的方法，返回true表示可以显示，返回false表示隐藏
    ...
    isLeaf: {type: Function as any as new() => TreePropsType["isLeaf"]},// 判断树节点是否为叶子节点的函数，仅在lazy模式有效
    getChildren: {type: Function as any as new() => TreePropsType["getChildren"]},// 加载子节点数据的函数，仅当 lazy 为true时有效
    ...
    isCheckable: {type: Function as any as new() => TreePropsType['isCheckable']},// 当即将选中树节点时，判断是否可以选中该树节点

    // 拖拽属性
    draggable: {type: Boolean},                                 // 是否可拖拽
    allowDrag: {type: Function as any as new() => TreePropsType["allDrag"]},// 判断节点是否可以拖拽
    allowDrop: {type: Function as any as new() => TreePropsType["allDrop"]},// 判断目标节点能够被放置
