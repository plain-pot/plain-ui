    ...
    <demo-row title="基本用法">
      <pl-form ref="form" v-model="form1.formData" contentWidth="400px" :disabled="disabled">
        <pl-form-item label="普通文本框" field="field1" required>
          <pl-input v-model="form1.formData.field1" placeholder="请输入文本"/>
        </pl-form-item>
        <pl-form-item label="普通文本域" field="field2" required>
          <pl-input v-model="form1.formData.field2" textarea placeholder="请选择"/>
        </pl-form-item>

        <pl-form-item label="数字框" field="field3" :rules="{max:100,message:'最大值100'}" required>
          <pl-number v-model="form1.formData.field3" :inputProps="{block:true}" placeholder="请输入数字"/>
          <template #suffix>
        ...
        <pl-form-item label="下拉选项" field="field6" required :rules="{message:'只能选择二级', options:'2'}">
          <pl-select v-model="form1.formData.field6" placeholder="请选择">
        ...
        <pl-form-item label="下拉选项:多选" field="field62" :rules="{required:true,min:1,message:'最少选择一项'}">
          <pl-select v-model="form1.formData.field62" multiple placeholder="请选择">
            <pl-select-option v-for="item in levelData" :label="item.levelName" :val="item.code" :key="item.code"/>
          </pl-select>
        </pl-form-item>

        <pl-form-item label="级联选择" field="field61" :rules="{required:true,message:'不能为空'}">
          <pl-cascade :data="treeData" labelField="name" keyField="id" childrenField="subs" v-model="form1.formData.field61" placeholder="请选择"/>
        </pl-form-item>
        ...
        <pl-form-item label="标签输入框" field="field12" required :rules="[{min:3,message:'最少输入3个标签'},{max:5,message:'最多5个标签'}]">
          <pl-tag-input v-model="form1.formData.field12" placeholder="请选择"/>
        </pl-form-item>
        ...
        <pl-form-item label="颜色选择器" field="field14" :rules="{required:true,message:'颜色不能为空!'}">
          <pl-color-picker v-model="form1.formData.field14" placeholder="请选择"/>
        </pl-form-item>
        <pl-form-item label="时间选择" field="field15" required>
          <pl-time v-model="form1.formData.field15" placeholder="请选择"/>
        </pl-form-item>
        <pl-form-item label="时间范围选择" :field="['field17','field18']" required>
          <pl-time v-model:start="form1.formData.field17" v-model:end="form1.formData.field18" range placeholder="请选择"/>
        </pl-form-item>

        <pl-form-item label="日期时间选择" field="field19" required>
          <pl-date v-model="form1.formData.field19" placeholder="请选择"/>
        </pl-form-item>

        <pl-form-item label="日期时间范围选择" :field="['field20','field21']" required>
          <pl-date v-model:start="form1.formData.field20" v-model:end="form1.formData.field21" range datetime placeholder="请选择"/>
        </pl-form-item>
